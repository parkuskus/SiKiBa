...
export async function submitWeight(input: WeightInput) {
  ...
  const sRow = { id: crypto.randomUUID(), userId: input.userId, tipe: 'weight', skor: kenaikan, kategori: trajectory === 'Normal' ? 'HIJAU' : trajectory === 'Kurang' ? 'KUNING' : 'MERAH', detail: { ...input, imt, targetKg, kenaikan, trajectory }, createdAt: new Date().toISOString() } as const
