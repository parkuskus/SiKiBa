...
export type DiaryInput = { userId: string; teks: string; mood: 1 | 2 | 3 | 4 | 5; tanggal?: string; judul?: string }
...
export async function submitDiary(input: DiaryInput) {
  ...
  const row = { id: globalThis.crypto?.randomUUID?.() ?? ("demo-" + Date.now() + "-" + Math.random().toString(36).slice(2,8)), userId: input.userId, tanggal, teks: input.teks.trim(), mood: input.mood, judul: input.judul?.trim() || undefined }
